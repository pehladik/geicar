...
#define REQUIRE(cond) { if (!(cond)) return false; }
	...
	static bool readNodeType(char &output, const char *&pathDef) {
		...
		if (nodeType && nodeType != '+' && nodeType != '-' && nodeType != '.' && nodeType != ',' &&
		    (nodeType < '0' || nodeType > '9')) {
			++pathDef;
			output = nodeType;
			return true;
		}
	...
	static void
	addArcApproximate(Contour &contour, Point2 startPoint, Point2 endPoint, Vector2 radius, double rotation, bool largeArc,
	                  bool sweep) {
	...
	static bool buildFromPath(Shape &shape, const char *pathDef, double size) {
		...
		while (nodeTypePreread || readNodeType(nodeType, pathDef)) {
			...
			while (*pathDef) {
				switch (nodeType) {
					...
					case 'm':
						...
						break;
					case 'Z':
					case 'z': REQUIRE(!contourStart);
						goto NEXT_CONTOUR;
					case 'L':
					case 'l': REQUIRE(readCoord(node, pathDef));
						if (nodeType == 'l')
							node += prevNode;
						contour.addEdge(new LinearSegment(prevNode, node));
						break;
					case 'H':
					case 'h': REQUIRE(readDouble(node.x, pathDef));
						if (nodeType == 'h')
							node.x += prevNode.x;
						contour.addEdge(new LinearSegment(prevNode, node));
						break;
					case 'V':
					case 'v': REQUIRE(readDouble(node.y, pathDef));
						if (nodeType == 'v')
							node.y += prevNode.y;
						contour.addEdge(new LinearSegment(prevNode, node));
						break;
					case 'Q':
					case 'q': REQUIRE(readCoord(controlPoint[0], pathDef));
						REQUIRE(readCoord(node, pathDef));
						if (nodeType == 'q') {
					...
					case 't':
						...
						break;
					case 'C':
					case 'c': REQUIRE(readCoord(controlPoint[0], pathDef));
					...
					case 'a': {
						...
						break;
					default: REQUIRE(!"Unknown node type");
				}
			...
			if (!contour.edges.empty() && prevNode != startPoint) {
				if ((contour.edges[contour.edges.size() - 1]->point(1) - contour.edges[0]->point(0)).length() <
				    ENDPOINT_SNAP_RANGE_PROPORTION * size)
