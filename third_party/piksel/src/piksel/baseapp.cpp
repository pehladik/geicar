...
// PUBLIC
	...
	void BaseApp::start() {
		...
		glVertexAttribPointer(a_fillColor, 4, GL_FLOAT, GL_FALSE, sizeof(ShaderRelevantState),
		                      (void *) offsetof(ShaderRelevantState, fillColor));
		glVertexAttribDivisor(a_fillColor, 1);

		GLuint a_strokeColor = 2;
		glEnableVertexAttribArray(a_strokeColor);
		glVertexAttribPointer(a_strokeColor, 4, GL_FLOAT, GL_FALSE, sizeof(ShaderRelevantState),
		                      (void *) offsetof(ShaderRelevantState, strokeColor));
		glVertexAttribDivisor(a_strokeColor, 1);

		GLuint a_tintColor = 3;
		glEnableVertexAttribArray(a_tintColor);
		glVertexAttribPointer(a_tintColor, 4, GL_FLOAT, GL_FALSE, sizeof(ShaderRelevantState),
		                      (void *) offsetof(ShaderRelevantState, tintColor));
		glVertexAttribDivisor(a_tintColor, 1);

		GLuint a_strokeWeight = 4;
		glEnableVertexAttribArray(a_strokeWeight);
		glVertexAttribPointer(a_strokeWeight, 1, GL_FLOAT, GL_FALSE, sizeof(ShaderRelevantState),
		                      (void *) offsetof(ShaderRelevantState, strokeWeight));
		glVertexAttribDivisor(a_strokeWeight, 1);

		GLuint a_textureIndex = 5;
		glEnableVertexAttribArray(a_textureIndex);
		glVertexAttribPointer(a_textureIndex, 1, GL_FLOAT, GL_FALSE, sizeof(ShaderRelevantState),
		                      (void *) offsetof(ShaderRelevantState, textureIndex));
		glVertexAttribDivisor(a_textureIndex, 1);

		GLuint a_modelMatrix = 6;
		glEnableVertexAttribArray(a_modelMatrix + 0);
		glVertexAttribPointer(a_modelMatrix + 0, 4, GL_FLOAT, GL_FALSE, sizeof(ShaderRelevantState),
		                      (void *) (offsetof(ShaderRelevantState, modelMatrix) + 0 * sizeof(glm::vec4)));
		glVertexAttribDivisor(a_modelMatrix + 0, 1);
		glEnableVertexAttribArray(a_modelMatrix + 1);
		glVertexAttribPointer(a_modelMatrix + 1, 4, GL_FLOAT, GL_FALSE, sizeof(ShaderRelevantState),
		                      (void *) (offsetof(ShaderRelevantState, modelMatrix) + 1 * sizeof(glm::vec4)));
		glVertexAttribDivisor(a_modelMatrix + 1, 1);
		glEnableVertexAttribArray(a_modelMatrix + 2);
		glVertexAttribPointer(a_modelMatrix + 2, 4, GL_FLOAT, GL_FALSE, sizeof(ShaderRelevantState),
		                      (void *) (offsetof(ShaderRelevantState, modelMatrix) + 2 * sizeof(glm::vec4)));
		glVertexAttribDivisor(a_modelMatrix + 2, 1);
		glEnableVertexAttribArray(a_modelMatrix + 3);
		glVertexAttribPointer(a_modelMatrix + 3, 4, GL_FLOAT, GL_FALSE, sizeof(ShaderRelevantState),
		                      (void *) (offsetof(ShaderRelevantState, modelMatrix) + 3 * sizeof(glm::vec4)));
		glVertexAttribDivisor(a_modelMatrix + 3, 1);

		GLuint a_textureMatrix = 10;
		glEnableVertexAttribArray(a_textureMatrix + 0);
		glVertexAttribPointer(a_textureMatrix + 0, 4, GL_FLOAT, GL_FALSE, sizeof(ShaderRelevantState),
		                      (void *) (offsetof(ShaderRelevantState, textureMatrix) + 0 * sizeof(glm::vec4)));
		glVertexAttribDivisor(a_textureMatrix + 0, 1);
		glEnableVertexAttribArray(a_textureMatrix + 1);
		glVertexAttribPointer(a_textureMatrix + 1, 4, GL_FLOAT, GL_FALSE, sizeof(ShaderRelevantState),
		                      (void *) (offsetof(ShaderRelevantState, textureMatrix) + 1 * sizeof(glm::vec4)));
		glVertexAttribDivisor(a_textureMatrix + 1, 1);
		glEnableVertexAttribArray(a_textureMatrix + 2);
		glVertexAttribPointer(a_textureMatrix + 2, 4, GL_FLOAT, GL_FALSE, sizeof(ShaderRelevantState),
		                      (void *) (offsetof(ShaderRelevantState, textureMatrix) + 2 * sizeof(glm::vec4)));
		glVertexAttribDivisor(a_textureMatrix + 2, 1);
		glEnableVertexAttribArray(a_textureMatrix + 3);
		glVertexAttribPointer(a_textureMatrix + 3, 4, GL_FLOAT, GL_FALSE, sizeof(ShaderRelevantState),
		                      (void *) (offsetof(ShaderRelevantState, textureMatrix) + 3 * sizeof(glm::vec4)));
		...
		describeShape(
				"{ 1, 0; (1, 0.55191502449; 0.55191502449, 1); 0, 1; (-0.55191502449, 1; -1, 0.55191502449); -1, 0; (-1, -0.55191502449; -0.55191502449, -1); 0, -1; (0.55191502449, -1; 1, -0.55191502449); # }",
				ellipse);
		...
		while (!glfwWindowShouldClose(window)) {
			...
			std::this_thread::sleep_for(std::chrono::duration<double>(
					start + SECONDS_PER_FRAME - glfwGetTime())); // TODO: would not be necessary with vsync
		}
#endif
	}
...
// PRIVATE
	...
	void BaseApp::mainLoop(Graphics &g) {
		...
		glBufferSubData(GL_ARRAY_BUFFER, 0, shaderRelevantStateVector.size() * sizeof(ShaderRelevantState),
		                shaderRelevantStateVector.data());
