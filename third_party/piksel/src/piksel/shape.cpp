...
namespace piksel {
	...
	int registerShape(msdfgen::Shape &msdfgenShape, Shape &pikselShape) {
		...
		for (std::vector<msdfgen::Contour>::const_iterator contour = msdfgenShape.contours.begin();
		     contour != msdfgenShape.contours.end(); ++contour) {
			for (std::vector<msdfgen::EdgeHolder>::const_iterator edge = contour->edges.begin();
		...
		glBindTexture(GL_TEXTURE_2D, msdf._texture);
		glTexSubImage2D(GL_TEXTURE_2D, 0, pikselShape.xoff, msdf.height - pikselShape.yoff - SHAPE_HEIGHT, SHAPE_WIDTH - 1,
		                SHAPE_HEIGHT - 1, GL_RGB, GL_UNSIGNED_BYTE, pixels);
