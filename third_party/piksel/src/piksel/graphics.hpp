...
namespace piksel {
	...
	class Graphics {
	public:
		Graphics(int &width, int &height, int &framebufferWidth, int &framebufferHeight, std::stack<State> &stateStack,
		         std::vector<ShaderRelevantState> &shaderRelevantStateVector, std::vector<Shape> &shapes)
				: width(width), height(height), framebufferWidth(framebufferWidth), framebufferHeight(framebufferHeight),
				  stateStack(stateStack), shaderRelevantStateVector(shaderRelevantStateVector), shapes(shapes) {
		}
