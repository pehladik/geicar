...
namespace piksel {
	...
	public:
		BaseApp(int width, int height, std::string title, bool fullscreen = false)
				: width(width), height(height), fullscreen(fullscreen), title(title), sboSize(1024 * sizeof(State)),
				  postfx_texture(0), devicePixelRatio(1.0f) {
		}
		...
		BaseApp(std::string title, bool fullscreen = false) : BaseApp(fullscreen ? -1 : 100, fullscreen ? -1 : 100, title,
		                                                              fullscreen) {
		}
