...
#pragma omp for
	...
	void
	generatePseudoSDF(Bitmap<float> &output, const Shape &shape, double range, const Vector2 &scale, const Vector2 &translate) {
		int contourCount = shape.contours.size();
		int w = output.width(), h = output.height();
		std::vector<int> windings;
		windings.reserve(contourCount);
...
#pragma omp for
	...
	void generateMSDF(Bitmap<FloatRGB> &output, const Shape &shape, double range, const Vector2 &scale, const Vector2 &translate,
...
#pragma omp for
			...
			for (int y = 0; y < h; ++y) {
				...
				for (int x = 0; x < w; ++x) {
					...
					for (int i = 0; i < contourCount; ++i, ++contour) {
						...
						double medMinDistance = fabs(
								median(r.minDistance.distance, g.minDistance.distance, b.minDistance.distance));
						if (medMinDistance < d) {
							d = medMinDistance;
							winding = -windings[i];
						}
	...
	void
	generateSDF_legacy(Bitmap<float> &output, const Shape &shape, double range, const Vector2 &scale, const Vector2 &translate) {
		int w = output.width(), h = output.height();
#ifdef MSDFGEN_USE_OPENMP
#pragma omp parallel for
	...
	void generatePseudoSDF_legacy(Bitmap<float> &output, const Shape &shape, double range, const Vector2 &scale,
...
#pragma omp parallel for
	...
	void generateMSDF_legacy(Bitmap<FloatRGB> &output, const Shape &shape, double range, const Vector2 &scale,
	                         const Vector2 &translate, double edgeThreshold) {
		int w = output.width(), h = output.height();
#ifdef MSDFGEN_USE_OPENMP
#pragma omp parallel for
#endif
