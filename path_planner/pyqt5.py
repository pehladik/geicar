    ...
    def mousePressEvent(self, QMouseEvent):
        if QMouseEvent.button() == Qt.LeftButton:
            ...
            if(self.curr_state=='normal'):
                ...
                self.setStyleSheet("background-color : yellow;border : none;")
            else:
                self.curr_state='normal'
            ...
            if(self.curr_state=='normal'):
                ...
                self.setStyleSheet("background-color : crimson;border : none;")
            elif(self.curr_state=='start_pt'):
                self.curr_state='end_pt'
                self.setStyleSheet("background-color : DodgerBlue;border : none;")
            else:
                self.curr_state='normal'
                self.setStyleSheet("background-color : white;border : none;")
...
class Fenetre(QWidget):
    def __init__(self):
        ...
        button = QPushButton('Infinity')
        button.clicked.connect(self.radar_infinite)
        hbox.addWidget(button)
        vbox.addLayout(hbox)
        hbox=QHBoxLayout()
        ...
        for color,state in zip(color_name,states):
            ...
            label.setText(state)
            label.setStyleSheet('color:'+color+';background-color : black;border : none;font-weight: bold;font: 14pt;')
    ...
    def radar_infinite(self):
        self.qTimer = QTimer()
        # set interval to 1 s
        self.qTimer.setInterval(50) # 1000 ms = 1 s
        # connect timeout signal to signal handler
        self.qTimer.timeout.connect(self.get_radar_obs)
        # start timer
        self.qTimer.start()
        ...
        try :
            if hasattr(self, 'rad'):
                ...
                with open("obstacles_real.txt", "r") as f:
                    data=f.read()
                self.rad=Radar(data,self.max_row,self.max_col,self.coeff,option="TXT")
                
            #data=data.replace('\n','')
            #CHANGER FONCTIONNEMENT
            self.rad.update()
            self.arr2d=self.rad.get_arr2d()
            #rad.print_arr_2d()
            ...
            for position in self.positions:
                ...
                if(self.arr2d[row][col].obstacle==True):
                    ...
                    btn.setStyleSheet("background-color : white;border : none;")
                    btn.curr_state='normal'
            btn=self.grid.itemAtPosition(0,int(0-self.rad.min)).widget()
            btn.setStyleSheet("background-color : crimson;border : none;")
            btn.curr_state='start_pt'
            self.cmp_error=0
            
        except:
            self.cmp_error=self.cmp_error+1
            if(self.cmp_error>=25):
                print("STOP")
                self.qTimer.stop()
        ...
        if(path!=None):
            ...
            for position in path:
                ...
                if((row==self.end.row and col==self.end.col) or (row==self.start.row and col==self.start.col)):
                    continue
                btn=self.grid.itemAtPosition(*position).widget()
                btn.setStyleSheet("background-color : blue;border : none;")
